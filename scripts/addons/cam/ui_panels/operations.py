...
class CAM_OPERATIONS_Panel(CAMButtonsPanel, Panel):
    ...
    def draw_operations_list(self):
        ...
        col.separator()
        col.operator("scene.cam_operation_move", icon="TRIA_UP", text="").direction = (
            "UP"
        )
        col.operator(
    ...
    def draw_presets(self):
        ...
        row.menu(
            "CAM_OPERATION_MT_presets", text=bpy.types.CAM_OPERATION_MT_presets.bl_label
        )
    ...
    def draw_calculate_path(self):
        ...
        self.layout.operator(
            "object.calculate_cam_path", text="Calculate Path & Export Gcode"
        )
        ...
        if self.op.strategy == "CURVE":
            if self.op.geometry_source == "OBJECT":
                ...
                self.layout.prop_search(
                    self.op, "collection_name", bpy.data, "collections"
                )
        else:
            if self.op.geometry_source == "OBJECT":
                self.layout.prop_search(self.op, "object_name", bpy.data, "objects")
                ...
                self.layout.prop_search(
                    self.op, "source_image_name", bpy.data, "images"
                )
