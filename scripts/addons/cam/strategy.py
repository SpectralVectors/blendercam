...
async def cutout(o):
    ...
    if o.first_down:  # each shape gets either cut all the way to bottom,
        ...
        for chunk in chunksFromCurve:
            dir_switch = (
                False  # needed to avoid unnecessary lifting of cutter with open chunks
            )
    ...
    for l in layers:
        ...
        if o.movement.helix_enter:
            ...
            for chi, ch in enumerate(lchunks):
                if not chunksFromCurve[chi].children:
                    ...
                    checkc = Circle(
                        helix_radius + c_offset, o.optimisation.circle_detail
                    )
        ...
        if o.movement.retract_tangential:
            ...
            for chi, ch in enumerate(lchunks):
                ...
                if (
                    chunksFromCurve[chi].parents == []
                    or len(chunksFromCurve[chi].parents) == 1
                ):
            ...
            for c in ob.data.splines:
                ...
                for p in c.points:
                    if o.drill_type == "ALL_POINTS":
                        chunks.append(
                            camPathChunk([(p.co.x + l.x, p.co.y + l.y, p.co.z + l.z)])
                        )
                ...
                if (
                    ...
                    chunks.append(
                        camPathChunk([(center[0] + l.x, center[1] + l.y, cz + l.z)])
                    )
...
async def medial_axis(o):
    ...
    if o.cutter_type == "VCARVE":
        ...
        raise CamException(
            "Only Ballnose and V-carve Cutters Are Supported for Medial Axis."
        )
        ...
        if ch.count() > 0:
            ...
            if chi < len(chunks) - 1 and chunks[chi + 1].count() > 0:
                ...
                if (
                    ...
                    o.machine_axes == "4" and vect.length < o.dist_between_paths * 2.5
                ):
