...
class VIEW3D_MT_PIE_Chains(Menu):
    ...
    def draw(self, context):
        ...
        column.separator()
        column.operator(
            "scene.cam_operation_move", icon="TRIA_UP", text=""
        ).direction = "UP"
        column.operator(
            "scene.cam_operation_move", icon="TRIA_DOWN", text=""
        ).direction = "DOWN"
        ...
        column.operator(
            "object.calculate_cam_path", text="Calculate Path & Export G-code"
        )
        if operation.valid:
        ...
        if operation.strategy == "CURVE":
            if operation.geometry_source == "OBJECT":
                ...
                column.prop_search(
                    operation, "collection_name", bpy.data, "collections"
                )
        else:
            if operation.geometry_source == "OBJECT":
                column.prop_search(operation, "object_name", bpy.data, "objects")
        ...
        row = column.row()
        row.template_list(
            "CAM_UL_chains", "", scene, "cam_chains", scene, "cam_active_chain"
        )
        ...
        if len(scene.cam_chains) > 0:
            ...
            if chain:
                ...
                column.operator(
                    "scene.cam_chain_operation_remove", icon="REMOVE", text=""
                )
                if len(chain.operations) > 0:
                    column.operator(
                        "scene.cam_chain_operation_up", icon="TRIA_UP", text=""
                    )
                    column.operator(
                        "scene.cam_chain_operation_down", icon="TRIA_DOWN", text=""
                    )
                ...
                if not chain.computing:
                    ...
                    column.operator(
                        "object.cam_export_paths_chain", text="Export Chain G-code"
                    )
                    column.operator(
                        "object.cam_simulate_chain", text="Simulate This Chain"
                    )
        ...
        column.scale_y = 2
        column.emboss = "NONE"
        column.operator("wm.call_menu_pie", text="", icon="HOME").name = (
            "VIEW3D_MT_PIE_CAM"
        )
