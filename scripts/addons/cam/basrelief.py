...
class BASRELIEF_Panel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        column.prop(br, 'bit_diameter')
        column.prop(br, 'pass_per_radius')
        column.prop(br, 'widthmm')
        column.prop(br, 'heightmm')
        column.prop(br, 'thicknessmm')

        box = layout.box()
        column = box.column(align=True)
        column.label(text="Justification")
        column.prop(br, 'justifyx')
        column.prop(br, 'justifyy')
        column.prop(br, 'justifyz')

        box = layout.box()
        column = box.column(align=True)
        column.label(text="Silhouette")
        column.prop(br, 'silhouette_threshold')
        column.prop(br, 'recover_silhouettes')
        if br.recover_silhouettes:
            column.prop(br, 'silhouette_scale')
            if br.advanced:
                column.prop(br, 'silhouette_exponent')
