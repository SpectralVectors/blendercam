...
class CamCurveOvercutsB(Operator):
    ...
    def poll(cls, context):
        return (
            context.active_object is not None and context.active_object.type == "CURVE"
        )
    ...
    def execute(self, context):
        ...
        diameter = (
            self.diameter * 1.002
        )  # make bit size slightly larger to allow cutter
        radius = diameter / 2
...
class CamOffsetSilhouete(Operator):
    ...
    def execute(self, context):
        ...
        if self.opencurve and ob.type == "CURVE":
            ...
            for (
                v
            ) in obj.data.vertices:  # extract X,Y coordinates from the vertices data
            ...
            dilated = line.buffer(
                self.offset, cap_style=1, resolution=16, mitre_limit=self.mitrelimit
            )  # use shapely to expand
            polygon_utils_cam.shapelyToCurve("dilation", dilated, 0)
        else:
            utils.silhoueteOffset(
                context, self.offset, int(self.style), self.mitrelimit
            )
        return {"FINISHED"}
