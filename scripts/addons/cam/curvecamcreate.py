    ...
    def execute(self, context):
        ...
        for s in shapes.geoms:
            ...
            rotated = affinity.rotate(
                lines, self.angle, use_radians=True
            )  # rotate using shapely
            translated = affinity.translate(
                rotated, xoff=centerx, yoff=centery
            )  # move using shapely
...
class CamCurveInterlock(Operator):
    ...
    def execute(self, context):
        print(
            len(context.selected_objects), "selected object", context.selected_objects
        )
        if len(context.selected_objects) > 0 and (
            context.active_object.type in ["CURVE", "FONT"]
        ):
            o1 = bpy.context.active_object
    ...
    def execute(self, context):
        ...
        bpy.ops.object.duplicate_move(
            ...
            TRANSFORM_OT_translate={
                "value": (0, -self.drawer_plate_thickness / 2, 0.0)
            },
        )
        ...
        bpy.ops.transform.transform(
            mode="TRANSLATION",
            value=(
                self.depth / 2 + 3 * self.width / 2 + 0.02,
                2 * self.height,
                0.0,
                0.0,
            ),
